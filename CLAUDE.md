# Teaching Tips Website

## Project goal
This project is for building a website focused on teaching tips.

The main purpose of this project is to:
- collect and organize useful teaching tips
- structure them clearly for website visitors
- build a simple, maintainable website
- improve the site over time with better content and design

## Language preference
- Reply in English by default unless I ask for another language.

## Working style
- Before making changes or running commands, briefly say what you are about to do in one line.
- Ask before doing anything risky, destructive, or hard to undo.
- Prefer practical, simple, and maintainable solutions.
- When multiple options exist, recommend one clear best option.

## Project focus
This project should stay focused on:
- website structure
- content organization
- implementation planning
- front-end development
- improving readability and usability

Avoid mixing this project with unrelated personal assistant tasks.

## Website direction
When suggesting or building the site:
- keep the first version simple
- make the content easy to expand later
- prefer clear navigation and clean structure
- organize teaching tips in a way that is useful for real educators
- prioritize readability over visual complexity

## Content rules
When working on teaching tips content:
- organize ideas clearly
- avoid messy or unstructured notes
- turn rough thoughts into polished content when possible
- use headings, bullet points, and short sections where helpful
- keep content practical and easy to scan

## Project organization
Keep files well organized.
When helpful, separate work into:
- content
- structure
- planning
- implementation
- assets

Prefer clean filenames and readable folder structure.

## Documentation
When creating project notes or planning documents, keep them clear and actionable.

Useful document types may include:
- content plans
- page structure notes
- feature plans
- design notes
- task breakdowns

## Development preferences
- Prefer simple and reliable approaches over unnecessary complexity.
- Keep the codebase easy to understand and edit later.
- Avoid overengineering.
- When building the first version, focus on a working foundation first.

## Safety
- Do not run risky commands without asking first.
- Do not make destructive changes without confirmation.
- Keep work limited to this project folder unless explicitly asked otherwise.
